...
figlet_font_name=${figlet_font_file##*/}
figlet_font_name=${figlet_font_name%.*}

mkdir -p "${figlet_font_name}"
...
do
    ...
    letter_ord=32; while [ "$letter_ord" -lt 127 ]
    do
        ...
        # Clear output file if we're about to write the first line
        if [ "$i" -eq 0 ]
        then
            true > "${figlet_font_name}/${letter_ord}.letter"
        fi
                                                 # v---------- remove rightmost char ----------v
        sed -e "$start_offset!d;s/[$hardblank]/ /g;:a" -e "s/\(.\)\(.*\)\1$/\2\1/;ta" -e "s/.$//;q" "$figlet_font_file" >> "${figlet_font_name}/${letter_ord}.letter"
